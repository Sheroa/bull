...
$.fn.Dialogs = $.Dialogs = function(opts) {
	if (this instanceof $) {
...
Dialog.M = function(opts) {
	...
	//弹窗id
	d.id = opts.id || "diglog_wrapper";
	//弹窗外部样式
	d.cls = opts.cls || "";
	d.css = opts.css || {};
	d.diglogWrapper = '<div id="' + d.id + '" class="dialog-wrapper"></div>';
	//遮罩背景
	...
	d.diginfo = K.ParseTpl(d.msgbody, $.extend({},
		d.tpl, {
			close: d.close,
			msg: d.msg
		}));
...
Dialog.M.prototype = {
	...
	Init: function() { // 弹窗
		...
		d.closeBtn.bind("click", function() {
			...
			if (d.closeAll) {
				$(".diglog-wrapper,.dialog-overlay").remove();
	...
	autoClose: function(times, auto) {
		...
		if (auto) {
			...
			K.getTime.countTimer[d.id] = window.setTimeout(function() {
